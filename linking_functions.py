...
# -*- coding: utf-8 -*-
"""
Created on Tue May  7 11:51:10 2024
...
import curve_fitting as cf
        ...
        # start by finding candidates (newborn cells) to link to each cell 
        # when it dies linking_candidates is a dict: 
        # {cellid: {set of cell ids}}. Keys indicate a cell that has died and 
        # value contains all cells that are candidates to link to key, 
        # depending on whether they meet the max_time_gap and max_distance 
        # requirements. No values in dict are empty sets.
        
def get_linking_candidates(track_dict, max_time_gap, max_distance):
...
def choose_linking_partners(linking_candidates, track_dict):
    ...
    for dead_cell_id in linking_candidates:
        ...
        if len(cell_1['t']) >= 3:
            t1_fit = cell_1['t'][-4:]
            x1 = cf.least_squares(2, cell_1['x'][-4:], t1_fit)
            y1 = cf.least_squares(2, cell_1['y'][-4:], t1_fit)
        else:
            x1 = cf.least_squares(1, cell_1['x'], cell_1['t'])
            y1 = cf.least_squares(1, cell_1['y'], cell_1['t'])
        ...
        for spawn_cell_id in linking_candidates[dead_cell_id]: 
            ...
            if len(cell_2['t']) >= 3:
                t2_fit = cell_2['t'][:4]
                x2 = cf.least_squares(2, cell_2['x'][:4], t2_fit)
                y2 = cf.least_squares(2, cell_2['y'][:4], t2_fit)
            else:
                x2 = cf.least_squares(1, cell_2['x'], cell_2['t'])
                y2 = cf.least_squares(1, cell_2['y'], cell_2['t'])
